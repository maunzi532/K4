...
public class SetTest
{
	...
	public static void main(String[] args)
	{
		...
		if(ergebnisA.size() <= 5)
			ergebnisA.stream().map(Karte::name).forEach(x -> System.out.println("\t" + x));
		else
			System.out.println("\tZu viele");
		System.out.println("Waffen:");
		var ergebnisW = setTest.waffen.findeKarten(input);
		if(ergebnisW.size() <= 5)
			ergebnisW.stream().map(Karte::name).forEach(x -> System.out.println("\t" + x));
		else
			System.out.println("\tZu viele");
		System.out.println("Gegner:");
		var ergebnisG = setTest.gegner.findeKarten(input);
		if(ergebnisG.size() <= 5)
			ergebnisG.stream().map(Karte::name).forEach(x -> System.out.println("\t" + x));
		else
			System.out.println("\tZu viele");
		System.out.println("Klassen:");
		var ergebnisK = setTest.klassen.findeKarten(input);
		if(ergebnisK.size() <= 5)
			ergebnisK.stream().map(Karte::name).forEach(x -> System.out.println("\t" + x));
		else
			System.out.println("\tZu viele");
